        ...
        try {
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                if (!empty($_SESSION['cart'])) {
                    // If they have items in cart, send them to checkout so they don't lose their place
                    header('Location: checkout.php');
                    exit();
                } elseif ($user['role'] === 'admin') {
                    // If no cart items and they are admin, send to dashboard
                    header('Location: admin/index.php');
                    exit();
                } else {
                    // Send normal users to the home page
                    header('Location: index.php');
                    exit();
                }
